 ...
 */
export async function queryObjectsAsync<T extends object>(
  ...
  if (!Utils.isEmpty(queryInfo.primary)) {
    ...
    if (!Utils.isEmpty(queryInfo.mandatory)) {
      candidates = filterObjects(candidates, queryInfo.mandatory);
    }
    queryResults = {
      objects: candidates,
  ...
  return filterQueryResult(queryResults, queryInfo.assistive || [], queryInfo.ordinal);
}
 ...
 * @returns objects which match the selectors in queryInfo
 */
export function queryObjects<T extends object>(
 ...
 */
export function filterQueryResult<T extends object>(queryResult: ObjectQueryResult<T>, assistive: Selector[], ordinal?: OrdinalSelector): ObjectQueryResult<T> | undefined {
  ...
  if (assistive.length > 0) {
    ...
    for (let i = assistive.length; i > 0; --i) {
      ...
      for (let j = 0; j < nCi.length; ++j) {
        ...
        const filtered_objects = filterObjects(candidates, assistive_selectors);
 ...
 */
export function filterObjects<T extends object>(objects: T[], selectors: Selector[]): T[] {
  const res: T[] = [];
  for (const obj of objects) {
    if (matchSelectors(obj, selectors)) {
      res.push(obj);
    }
  }
  return res;
 ...
 * @returns {boolean}
 */
export function matchSelectors<T extends object>(obj: T, selectors: Selector[]): boolean {
 ...
 * @returns {boolean}
 */
export function matchSelector<T extends object>(obj: T, selector: Selector): boolean {
  ...
  if (matchType === 'has') {
    return hasSelectorKey(obj, selector);
  }
  else if (matchType === 'hasNot') {
    return !hasSelectorKey(obj, selector);
  }
  else {
    const actualValue = getValueBySelector(obj, selector);
    return matchValue(matchType, selector.value, actualValue);
  }
}
 ...
 */
export function hasSelectorKey<T extends object>(obj: T, selector: Selector): boolean {
  const key = selector.name;
 ...
 */
export function getValueBySelector<T extends object>(obj: T, selector: Selector): string | number | boolean | undefined | null {
  const key = selector.name;
