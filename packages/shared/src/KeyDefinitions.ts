...
export class KeyDefinitionUtils {

  static isKeyboardModifier(key: string): key is KeyboardModifier {
    return ['Alt', 'Control', 'Meta', 'Shift'].includes(key);
  }

  static getKeyDescription(key: string, pressedModifiers: Set<KeyboardModifier>): KeyDescription | null {
    if (key === 'ControlOrMeta') {
      key = BrowserUtils.isMacOS() ? 'Meta' : 'Control';
    }
    const keyString = key;
    const KeyDefinitions = KeyDefinitionUtils.KeyDefinitions;
   ...
   */
  static get KeyDefinitions() {
    if (!KeyDefinitionUtils.keyDefinitions) {
      KeyDefinitionUtils.keyDefinitions = KeyDefinitionUtils.buildLayoutClosure();
    }
    return KeyDefinitionUtils.keyDefinitions;
  }
  /**
   * US Keyboard Layout
   ...
   */
  private static USKeyboardLayout: Record<string, KeyDefinition> = {
    // Functions row
  ...
  private static keyDefinitions?: Map<string, KeyDescription>;
  /**
   * 
   ...
   */
  private static buildLayoutClosure(layout?: Record<string, KeyDefinition>): Map<string, KeyDescription> {
    layout = layout ?? KeyDefinitionUtils.USKeyboardLayout;
