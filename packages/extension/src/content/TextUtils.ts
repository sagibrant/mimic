import { NameValuePair, Utils } from "@/common/Common";
...
export class TextUtils {
     ...
     */
    static getNodeTextItems(node: Node, filter: TextFilter): NodeTextItem[] {
        ...
        if (filter) {
            ...
            if (Array.isArray(filter.includes) && filter.includes.length > 0) {
                included = filter.includes.some((nodeFilter) => {
                    return TextUtils.matchDomNodeFilter(node, nodeFilter);
                });
            }
     ...
     */
    static getNodeTextItemForTextNode(node: Node, visibilityLevel: 'char' | 'word' | 'none'): NodeTextItem | null {
        ...
        for (let i = 0; i < rects.length; i++) {
            ...
            for (; charIndex < text.length; charIndex++) {
                ...
                if (isSameLine) {
                    if (charValue === ' ') {
                        if (chars.length > 0) {
                            let wordTextItem = TextUtils.tryGetOuterTextItem(chars, 'word');
     ...
     * @returns {NodeTextItem|null}
     */
    static getNodeTextItemForElementNode(node: Node, visibilityLevel: 'char' | 'word' | 'none'): NodeTextItem | null {
     ...
     */
    static tryGetOuterTextItem(textItems: TextItem[], type: TextItemType): TextItem | null {
     ...
     * @returns {boolean}
     */
    static matchDomNodeFilter(node: Node, filter: DomNodeFilter): boolean {
        if (!node) {
            return false;
        }
        if (!filter) {
