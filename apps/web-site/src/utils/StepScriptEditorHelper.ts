...
export class StepScriptEditorHelper {
  ...
  static getTypeMethods(typeName: string): Array<MethodDefinition> {
    ...
    const type = JSON.parse(JSON.stringify(StepScriptEditorHelper.TypeDefinitions[typeName]));
    ...
    if (type.extends) {
      for (const baseType of type.extends) {
        ...
        const extendedMethods = JSON.parse(JSON.stringify(StepScriptEditorHelper.getTypeMethods(baseTypeName)));
        if (runtimeType) {
