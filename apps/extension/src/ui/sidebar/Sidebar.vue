<template>
  <div class="sidebar-container">
    <Toast position="bottom-right" group="br" class="text-sm p-2 max-w-xs" />
    <ConfirmDialog :style="{ width: '20rem' }"></ConfirmDialog>
    <Dialog v-model:visible="isAddTaskNodeDialogVisible" modal
      :header="t('sidebar_btn_action_tree_add_node_dialog_header')" :style="{ width: '20rem' }">
      <Form v-slot="{ nodeType, nodeName }: { [key: string]: any }" :resolver="resolver" :initialValues="initialValues"
        @submit="onAddTaskNodeSubmit" class="flex justify-center flex-col gap-4">
        <div class="flex items-center gap-4 mb-4">
          <label for="add_new_node_type" class="font-semibold w-24">{{
            t('sidebar_btn_action_tree_add_node_dialog_label_type') }}</label>
          <Select id="add_new_node_type" name="nodeType" :options="taskNodeTypes" optionLabel="name" optionValue="code"
            defaultValue="task" class="flex-auto" fluid />
        </div>
        <div class="flex items-center gap-4 mb-8">
          <label for="add_new_node_name" class="font-semibold w-24">{{
            t('sidebar_btn_action_tree_add_node_dialog_label_name') }}</label>
          <InputText id="add_new_node_name" name="nodeName" class="flex-auto" autocomplete="off" fluid />
        </div>
        <Message v-if="nodeType?.invalid" severity="error" size="small" variant="simple" class="flex justify-end">{{
          nodeType?.error?.message }}</Message>
        <Message v-if="nodeName?.invalid" severity="error" size="small" variant="simple" class="flex justify-end">{{
          nodeName?.error?.message }}</Message>
        <div class="flex justify-end gap-2">
          <Button type="submit" :label="t('sidebar_dialog_ok')" severity="secondary"></Button>
          <Button type="button" :label="t('sidebar_dialog_cancel')" @click="handleAddTaskNodeCanceled"></Button>
        </div>
      </Form>
    </Dialog>
    ...
    <main class="sidebar-middle">
      ...
      <div class="steps-panel" @click="handleStepsPanelClick">
        ...
        <div class="steps-controls">
          ...
          <span class="menu-divider"></span>
          <button class="command-btn" :disabled="!(isIdle)" @click="openAIDialog"
...
// const addTaskNodeName = ref('');
const taskNodeTypes = ref([
 ...
 */
const showNotificationMessage = (message: string, timeout: number = 3000, severity: 'success' | 'info' | 'warn' | 'error' = 'info', summary: string = '') => {
  if (summary) {
    primevueToast.add({ severity: severity, summary: summary, detail: message, group: 'br', life: timeout });
  }
  else {
    primevueToast.add({ severity: severity, detail: message, group: 'br', life: timeout });
  }
};
...
/**
 * handle the AddTaskNode callback, display the Dialog
 */
...
/**
 * Add new task node (group or task) after click on the ok button
 */
const onAddTaskNodeSubmit = (event: {
  originalEvent: Event;
  ...
  const nodeType = event.values['nodeType'] as string;
  const nodeName = event.values['nodeName'] as string;
  if (!nodeType) {
  ...
  isAddTaskNodeDialogVisible.value = false;
...
/**
 * cancel the add task node action
 */
const handleAddTaskNodeCanceled = () => {
  isAddTaskNodeDialogVisible.value = false;
}
...
const handleDeleteTaskNode = () => {
  ...
  primevueConfirm.require({
    ...
    message: t('sidebar_btn_action_tree_delete_node_confirm_message'),
    icon: 'pi pi-exclamation-triangle',
    rejectProps: {
      label: t('sidebar_confirm_cancel'),
      severity: 'secondary',
...
const handleRemoveStep = () => {
  ...
  primevueConfirm.require({
    header: t('sidebar_btn_action_steps_delete_step_confirm_header'),
    message: t('sidebar_btn_action_steps_delete_step_confirm_message'),
    icon: 'pi pi-exclamation-triangle',
    rejectProps: {
      label: t('sidebar_confirm_cancel'),
      severity: 'secondary',
...
/**
 * toggle the AI Dialog visibility
 */
const openAIDialog = () => {
...
const handleRunScript = async (script: string) => {
  ...
  if (runningScript.value.length > 0) {
    showNotificationMessage('Another script is running', 3000, 'warn');
    return;
  }
    ...
    if (result) {
      ...
      if (typeof result === 'object') {
        try {
          ...
          showNotificationMessage(`Script run result: ${result}`);
        }
      }
      else {
        showNotificationMessage(`Script run result: ${result}`);
