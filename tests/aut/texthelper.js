...
class TextFilter {
    ...
    includeDefaultElements() {
        ...
        filter_elem_input.properties = [new NameValuePair('nodeType', Node.ELEMENT_NODE), new NameValuePair('nodeName', 'INPUT')];
        this.includes.push(filter_elem_input);
        // include textarea element
        let filter_elem_textarea = new DomNodeFilter();
        filter_elem_textarea.properties = [new NameValuePair('nodeType', Node.ELEMENT_NODE), new NameValuePair('nodeName', 'TEXTAREA')];
        this.includes.push(filter_elem_textarea);
        // include select element
        let filter_elem_select = new DomNodeFilter();
        filter_elem_select.properties = [new NameValuePair('nodeType', Node.ELEMENT_NODE), new NameValuePair('nodeName', 'SELECT')];
...
class TextHelper {
    ...
    getNodeTextItems(node, filter) {
        ...
        if (typeof (ShadowRoot) !== 'undefined' && node.shadowRoot instanceof ShadowRoot &&
            node.shadowRoot && node.shadowRoot.mode === 'open' &&
            node.shadowRoot.childNodes && node.shadowRoot.childNodes.length > 0) {
