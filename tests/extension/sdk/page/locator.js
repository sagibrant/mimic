...
if (browser) {
  ...
  console.log("await browser.window({ lastFocused: true }).page().all()", lastFocusedWindowPages, lastFocusedWindowPages.length);
  ...
  try {
    ...
    const error_page = await browser.page({ index: 0 }).nth(count + 1).get();
...
if (window) {
  ...
  try {
    ...
    const error_page = await window.page({ index: 0 }).nth(count + 1).get();
