...
const func = async (newPage) => {
  ...
  try {
    ...
    let count = await newPage.element('button.el-button.el-button--default.el-button--small.el-button--primary').count();
    if (count > 0) {
      await newPage.element('button.el-button.el-button--default.el-button--small.el-button--primary').first().highlight();
      await newPage.element('button.el-button.el-button--default.el-button--small.el-button--primary').first().click({ mode: "cdp" });
      await wait(3000);
    }
    ...
    while (lastTime < duration) {
      ...
      if (count > 0) {
        const textContent = await newPage.element('div.title-list > .chapter-title > .chapter-percentage').last().textContent();
        if (textContent === '100%') {
          break;
        }
      }
